...
module Advanced =
    ...
    let my_prg = """
    let f1 a1 = let f2 a2 = let f3 a3 = let f4 a5 = if a5 < 0 then 0 else match a5 with | 0 -> 0 | 3 -> if s = "questa     è     una     stringa     con in let if elif else ..." then 4 / 2 else 0 | 4 -> match a5 with | 0 -> 4 | _ -> 6 in f4 a3 in f3 a2 in f2 a1

    let f1 a1 = match a5 with | 0 -> let a b = match b with | 0 -> 1 | 1 -> if a = 3 then 2 else 0 in a 5 | 3 -> if 5 > 3 then 4 / 2 else 0 | 4 -> 0
    """

module Extreme =
    let autoindentazione = "\n" + System.IO.File.ReadAllText("../../Formatter.fs") + "\n"
...
let extreme = [E.autoindentazione] |> flatten

let all = basic + advanced + extreme
